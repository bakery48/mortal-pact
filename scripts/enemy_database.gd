...
static func zako_pool() -> Array:
	...
	return [
		...
		_enemy("スケルトン", 50, [_move(A, 7), _move(A, 10)], E.EARTH),
		...
		_enemy("毒蛇", 42, [_move(P, 3), _move(A, 7), _move(A, 9)], E.WATER),
		_enemy("サラマンダー", 50, [_move(A, 10), _move(A, 16)], E.FIRE),
		_enemy("フェンリル", 46, [_move(A, 9), _move(B, 3), _move(A, 13)], E.WIND),
		_enemy("ゴーレム", 62, [_move(D, 10), _move(A, 9)], E.EARTH),
		_enemy("ウィスプ", 34, [_move(B, 3), _move(A, 9), _move(A, 7)], E.FIRE),
...
static func elite_pool() -> Array:
	...
	return [
		...
		_enemy("闇の魔女", 80, [_move(A, 10), _move(H, 14), _move(A, 14)], E.WIND),
		_enemy("ガーゴイル", 95, [_move(D, 12), _move(A, 15), _move(A, 9)], E.EARTH),
		_enemy("地獄の番犬", 84, [_move(A, 12), _move(A, 12), _move(B, 5)], E.FIRE),
		_enemy("邪毒の妖蛆", 78, [_move(P, 5), _move(A, 11), _move(D, 10)], E.WATER),
		_enemy("ケルベロス", 95, [_move(A, 14), _move(A, 22), _move(B, 5)], E.FIRE),
		_enemy("ミノタウロス", 100, [_move(A, 13), _move(B, 4), _move(A, 19)], E.EARTH),
		_enemy("ヴァンパイア", 88, [_move(A, 12), _move(H, 12), _move(A, 18)], E.WIND),
	]
...
static func boss_pool() -> Array:
	...
	var E := MonsterData.Element
	return [
		_enemy("死の騎士", 150, [_move(A, 16), _move(D, 12), _move(A, 24), _move(B, 4)], E.EARTH, true),
		_enemy("双頭の竜", 175, [_move(A, 14), _move(A, 14), _move(B, 6), _move(A, 28)], E.FIRE, true),
		_enemy("深淵の王", 200, [_move(P, 6), _move(A, 18), _move(H, 20), _move(A, 26)], E.WATER, true),
	]
